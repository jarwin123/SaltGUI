...
export class Documentation {
  ...
  constructor (pRouter, pCommandBox) {
    ...
    this._manualRunMenuHtmlDocRun = this._manualRunMenuHtmlDocRun.bind(this);

    pCommandBox.cmdmenu.addMenuItem(
      this._manualRunMenuSysDocPrepare,
      this._manualRunMenuSysDocRun);
    pCommandBox.cmdmenu.addMenuItem(
      this._manualRunMenuHtmlDocPrepare,
      this._manualRunMenuHtmlDocRun);
    ...
    const localTestProviders = this.router.api.getLocalTestProviders();

    const myThis = this;
    localTestProviders.then(pData => {
      myThis._handleLocalTestProviders(pData);
    }, pData => {
      // VOID
    });
  }
  ...
  _handleLocalTestProviders(data) {
  ...
  _manualRunMenuHtmlDocRun(menuitem) {
    ...
    for(const m of concreteModules) {
      ...
      // level 2
      if(cmd.length >= 2)
        html += "<p><a href='" + Documentation.DOCUMENTATION_URL + cmd[0] + "/all/salt." + cmd[0] + "." + m + ".html' target='_blank'>Module '" + (cmd[0] + "." + m).replace(/^modules[.]/, "") + "'</a>" + Documentation.EXTERNAL_LINK + "</p>";

      // level 3
      if(cmd.length >= 3)
        html += "<p>" + indent + "<a href='" + Documentation.DOCUMENTATION_URL + cmd[0] + "/all/salt." + cmd[0] + "." + m + ".html#salt." + cmd[0] + "." + m + "." + cmd[2] + "' target='_blank'>Function '" + (cmd[0] + "." + m + "." + cmd[2]).replace(/^modules[.]/, "") + "'</a>" + Documentation.EXTERNAL_LINK + "</p>";
    }
